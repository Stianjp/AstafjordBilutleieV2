...
export async function sendBookingEmails({ customer, booking, car, pickup, delivery, thirdParty }) {
  ...
  if (customerSend?.error) {
    console.error("Failed to send customer booking email", customerSend.error);
  }
  ...
  if (adminSend?.error) {
    console.error("Failed to send admin booking email", adminSend.error);
  }
  ...
  if (thirdPartyContact?.email) {
    ...
    const thirdPartySend = await resend.emails.send({
      from: fromEmail,
      to: thirdPartyContact.email,
      subject: thirdPartySubject,
      html: thirdPartyHtml
    });
    if (thirdPartySend?.error) {
      console.error("Failed to send third-party booking email", {
        error: thirdPartySend.error,
...
export async function sendBookingDecisionEmail({ customer, booking, status }) {
  ...
  if (customerSend?.error) {
    console.error("Failed to send customer decision email", customerSend.error);
  }
  ...
  if ((status === "approved" || status === "rejected") && adminEmail) {
    ...
    if (adminSend?.error) {
      console.error("Failed to send admin decision email", adminSend.error);
    }
  }
  ...
  if ((status === "approved" || status === "rejected") && thirdParty?.email) {
    ...
    if (thirdPartySend?.error) {
      console.error("Failed to send third-party decision email", {
        error: thirdPartySend.error,
