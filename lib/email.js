...
const resolveLanguage = (language) => (language === "en" ? "en" : "no");

const contractIntroText = ({ thirdParty, language }) => {
  const lang = resolveLanguage(language);
  const t = translations[lang];
  if (!thirdParty) {
    return t.contract.intro;
  }
  return t.contract.introThirdParty
...
const buildContractHtml = ({ customer, booking, pickup, delivery, car, thirdParty, language }) => {
  const lang = resolveLanguage(language || booking?.contract_language);
  const t = translations[lang];
  const discountAmount = Number(booking.discount_amount || 0);
  const dailyDeductibleFee = calculateDeductibleReductionDailyFee(booking);
  const deductibleSelected = booking.deductible_reduction_selected ? t.contract.yes : t.contract.no;
  const thirdPartyName = formatThirdPartyDisplayName(thirdParty);
  const obligations = (t.contract.obligations || []).map((line) => {
  ...
  const exceptions = (t.contract.deductibleReductionExceptions || []).map((line) => `<p>${line}</p>`).join("");
  const terms = (t.contract.terms || []).map((line) => `<p>${line}</p>`).join("");

  return `
    <p>${contractIntroText({ thirdParty, language: lang })}</p>
    ...
    ${customer.address_line_2 ? `<p>${t.contract.address2}: ${customer.address_line_2}</p>` : ""}
    <p>${t.contract.postalCode}: ${customer.postal_code || "-"}</p>
    <p>${t.contract.region}: ${customer.region || "-"}</p>
    ${thirdParty ? `<p>${t.contract.onBehalfOf}: ${thirdPartyName}</p>` : ""}
    <p>${t.contract.pickup}: ${pickup?.name || "-"}</p>
    <p>${t.contract.delivery}: ${delivery?.name || "-"}</p>
    <p>${t.contract.start}: ${booking.start_date || "-"} ${t.contract.timePrefix} ${booking.start_time || "-"}</p>
    <p>${t.contract.end}: ${booking.end_date || "-"} ${t.contract.timePrefix} ${booking.end_time || "-"}</p>
    ...
    <p>${t.contract.deductibleReductionChoice}: ${deductibleSelected}</p>
    ...
    <p>${t.contract.period}: ${booking.days || "-"} ${t.labels.daysLabel}</p>
    ${discountAmount > 0 ? `<p>${t.labels.discountLabel}: -${Math.round(discountAmount)} NOK</p>` : ""}
    <p><strong>${t.contract.total}: ${booking.calculated_price || "-"} NOK</strong></p>
    <p>${t.contract.freeKm}: 200 km</p>
    <p>${t.contract.extraKm}</p>
    <p>${t.contract.responsibility}</p>
    <p>${t.contract.obligationsTitle}</p>
    ${obligations}
    <p>${t.contract.deductibleReductionTitle}</p>
    <p>${t.contract.deductibleReductionInfo.replace("{fee}", String(dailyDeductibleFee))}</p>
    ${booking.deductible_reduction_selected ? `<p>${t.contract.deductibleReductionAccepted.replace("{fee}", String(dailyDeductibleFee))}</p>` : ""}
    <p>${t.contract.deductibleReductionExceptionsIntro}</p>
    ${exceptions}
    <p>${t.contract.cancellationPolicyTitle}</p>
    <p>${t.contract.cancellationPolicyText}</p>
    <p>${t.contract.termsTitle}</p>
    ${terms}
  `;
};
...
export async function sendBookingEmails({ customer, booking, car, pickup, delivery, thirdParty, language }) {
  ...
  const customerHtml = `
    ...
    <p>${lang === "en" ? "We have received your booking request." : "Vi har mottatt bookingforesporselen din."}</p>
    ...
    ${thirdPartyContact ? `<p><strong>${lang === "en" ? "Rented on behalf of" : "Utleies pa vegne av"}:</strong> ${formatThirdPartyDisplayName(thirdPartyContact)}</p>` : ""}
...
export async function sendBookingDecisionEmail({ customer, booking, status }) {
  ...
  const resend = new Resend(resendKey);
  const lang = resolveLanguage(booking?.contract_language);
