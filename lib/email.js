...
export async function sendInsuranceMileageAlert({ car, remainingKm, annualLimit }) {
  ...
  const subject = `Astafjord bilutleie: ${model} - ${normalizedRemaining}km igjen av kjorelengde`;
  const html = `
    <p>${model} med kjennemerke: ${regNumber}</p>
    <p>Har ${normalizedRemaining} km igjen av ${normalizedLimit} km totalt i sin arlige kilometer pa forsikring.</p>
    <p>Naverende km-stand: ${currentKm} km.</p>
  `;
